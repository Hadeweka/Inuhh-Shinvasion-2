...
template <class T> static void ruby_free_data(mrb_state* mrb, void* object_ptr) {

	reinterpret_cast<T*>(object_ptr)->~T();

}

template <class T> void ruby_convert_to_instance_variable(mrb_state* mrb, mrb_value self, const char* var_c_str, const char* data_type_c_str, T* new_object) {

	static const struct mrb_data_type data_type = {

		data_type_c_str, ruby_free_data<T>
